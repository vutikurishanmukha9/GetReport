...
def _esc(value: Any) -> str:
    """Escape user/data-derived text for safe use in ReportLab Paragraph XML."""
...
def _build_title_page(filename: str, styles: dict[str, ParagraphStyle]) -> list[Flowable]:
    ...
    story.append(Paragraph(f"Source: {_esc(filename)}", styles["ReportSubtitle"]))
...
def _build_semantic_analysis_section(
    ...
    domain_text = f"<b>Detected Domain:</b> {_esc(domain_name)} ({int(confidence * 100)}% confidence)"
    if matched_keywords:
        domain_text += f"<br/><i>Matched patterns: {_esc(', '.join(matched_keywords[:8]))}</i>"
    ...
    if suggestions:
        ...
        for i, sugg in enumerate(suggestions[:5], 1):
            ...
            if len(cols) >= 2:
                sugg_text = f"{i}. <b>{_esc(cols[0])}</b> ↔ <b>{_esc(cols[1])}</b>: {_esc(reason)}"
...
def _build_executive_summary(
    ...
    if "summary" not in analysis_results or not analysis_results["summary"]:
        ...
        for col_name, metrics in summary.items():
            story.append(Paragraph(f"<i>{_esc(col_name)}</i>", styles["TableCaption"]))
