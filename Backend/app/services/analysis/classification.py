...
def classify_numeric_columns(df: pl.DataFrame, numeric_cols: list[str]) -> dict[str, list[str]]:
    """
    Classify numeric columns into semantic categories with sequence and distribution precision:
    - analytical: Real numeric data suitable for correlation/distribution (e.g., sales, price)
    - id_like: Likely identifiers (high uniqueness, sequential auto-increment) - exclude from analysis
    ...
    for col in numeric_cols:
        ...
        # Check 3: Strictly Sequential Auto-Incrementing IDs
        try:
            if is_int_type and df.height >= 10:
                non_null = df[col].drop_nulls()
                if non_null.len() >= 10:
                    diffs = non_null.diff().drop_nulls()
                    if (diffs == 1).all():
                        reasons.append("strictly_sequential_auto_increment")
        except Exception:
            pass
        
        # Check 4: Excel serial date range detection
        try:
            non_null = df[col].drop_nulls()
            if non_null.len() > 0:
        ...
        if "low_variance" in reasons:
            ...
            should_exclude = True
        elif "strictly_sequential_auto_increment" in reasons:
            should_exclude = True
